...
bool XYinBox(int x, int y ,int Kx, int Krows_down, int Kwidth){ // use DrawKey type key setting
    ...
    int v=Keyboard_Y + (30*Krows_down*KBD_size);
    int height=25;
...
bool KeyOver(int x, int y, char* Key, int type){ //char array version
  bool Keyfound=false;
   ...
   if ((y > Keyboard_Y) && (y < (Keyboard_Y+25))) { //a, Keyboard_Y, 20, 25,
    //top row  
   // Serial.printf(" In TOP ROW  KBD_size;(%i) x%i y%i \n",KBD_size,x,y);
    for (int z = 0; z < 10; z++) {
  ...
  if ((y > (Keyboard_Y + (30*KBD_size))) && (y < (Keyboard_Y + (30*KBD_size)+25))) { //Keyboard_Y + (30*KBD_size), 20, 25, 1, WHITE);
    //middle row
  ...
  if ((y > (Keyboard_Y + (60*KBD_size))) && (y < (Keyboard_Y + (60*KBD_size)+25))) {
    //bottom row
    for (int z = 0; z < 8; z++) {
